...
var insertClaimSchema = createInsertSchema(claims2).omit({
  id: true,
  userId: true,
  status: true,
  createdAt: true,
...
var documents = pgTable("documents", {
  id: serial("id").primaryKey(),
  claimId: integer("claim_id").notNull().references(() => claims2.id, { onDelete: "cascade" }),
...
var insertAuditLogSchema = createInsertSchema(auditLogs2).omit({
  id: true,
  timestamp: true
});
var usersRelations = relations(users, ({ many }) => ({
  ...
  auditLogs: many(auditLogs2)
}));
var claimsRelations = relations(claims2, ({ one, many }) => ({
  user: one(users, {
    fields: [claims2.userId],
    references: [users.id]
  }),
...
var auditLogsRelations = relations(auditLogs2, ({ one }) => ({
  user: one(users, {
    fields: [auditLogs2.userId],
    references: [users.id]
  })
}));
...
// server/storage.ts
import { eq as eq2, desc, sql } from "drizzle-orm";
var DatabaseStorage = class {
  ...
  async updateUserLastLogin(userId) {
    const [user] = await db.update(users).set({ lastLoginAt: /* @__PURE__ */ new Date() }).where(eq2(users.id, userId)).returning();
    return user;
  }
  async updateUserVerification(userId, isVerified) {
    const [user] = await db.update(users).set({ isVerified }).where(eq2(users.id, userId)).returning();
    return user;
  }
  // Claim operations
  async getClaim(id) {
    const [claim] = await db.select().from(claims2).where(eq2(claims2.id, id));
    return claim;
  }
  async getClaimsByUserId(userId) {
    return await db.select().from(claims2).where(eq2(claims2.userId, userId));
  }
  async createClaim(claim) {
    const now = /* @__PURE__ */ new Date();
    const [newClaim] = await db.insert(claims2).values({
      ...claim,
      createdAt: now,
      updatedAt: now
    }).returning();
  ...
  async updateClaimAnalysis(id, analysis) {
    const [updatedClaim] = await db.update(claims2).set({ aiAnalysis: analysis, updatedAt: /* @__PURE__ */ new Date() }).where(eq2(claims2.id, id)).returning();
    return updatedClaim;
  }
  // Document operations
  ...
  async getDocumentsByClaimId(claimId) {
    return await db.select().from(documents).where(eq2(documents.claimId, claimId));
  }
  ...
  async getChatThread(id) {
    const [thread] = await db.select().from(chatThreads).where(eq2(chatThreads.id, id));
    return thread;
  }
  async getChatThreadsByUserId(userId) {
    return await db.select().from(chatThreads).where(eq2(chatThreads.userId, userId)).orderBy(desc(chatThreads.createdAt));
  }
  ...
  async getChatMessages(threadId, limit = 100) {
    return await db.select().from(chatMessages).where(eq2(chatMessages.threadId, threadId)).orderBy(desc(chatMessages.createdAt)).limit(limit);
  }
  ...
  async updateChatThreadStatus(threadId, status) {
    ...
    }).where(eq2(chatThreads.threadId, threadId)).returning();
    return updatedThread;
  }
  // Document analysis operations
  async getDocumentAnalysisResult(id) {
    const [result] = await db.select().from(documentAnalysisResults).where(eq2(documentAnalysisResults.id, id));
    return result;
  }
  async getDocumentAnalysisResultsByDocumentId(documentId) {
    return await db.select().from(documentAnalysisResults).where(eq2(documentAnalysisResults.documentId, documentId)).orderBy(desc(documentAnalysisResults.createdAt));
  }
  ...
  async createAuditLog(log) {
    const [newLog] = await db.insert(auditLogs2).values({
      ...log,
  ...
  async getAuditLogsByUserId(userId, limit = 100) {
    return await db.select().from(auditLogs2).where(eq2(auditLogs2.userId, userId)).orderBy(desc(auditLogs2.timestamp)).limit(limit);
  }
};
...
async function registerRoutes(app2) {
  ...
  app2.post("/api/chat/threads", async (req, res) => {
    try {
      ...
      const username = req.user && "username" in req.user ? req.user.username : "User";
            ...
            if (data.userId) {
              try {
                await storage.createAuditLog({
                  ...
                  resourceId: `chat-${Date.now()}`,
                  ip: "websocket",
                  // No direct access to IP from WS
                  userAgent: "websocket",
                  // No direct access to user agent
...
(async () => {
  ...
  if (isProduction) {
    const clientDistPath = path3.resolve(process.cwd(), "client/dist");
